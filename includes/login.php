...
if( empty($user_uid) || empty($user_password) ){ header("Location: ../index.php?user=ERROR"); exit();}

//checking if the username or email exists on our table
$query = "SELECT * FROM users WHERE user_uid = '$user_uid';";
if(mysqli_num_rows( mysqli_query($connection, $query) )  <= 0 ){ header("Location: ../index.php?user=ERROR"); exit();}
